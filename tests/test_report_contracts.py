...
def _fixture_report() -> BenchmarkReport:
    return BenchmarkReport(
        suite_name="nuDemo contract fixture",
        dataset={
            "samples": 32,
            "scenes": 4,
            "provider": "real",
...
def _report_path_for_test(tmpdir: str) -> Path:
    if REPORT_PATH.exists():
        return REPORT_PATH
    report, _, _ = export_report_bundle(_fixture_report(), Path(tmpdir))
    return report
...
class ReportContractTests(unittest.TestCase):
    def test_current_report_shape_and_bundle_exports(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            report_path = _report_path_for_test(tmpdir)
    ...
    def test_observability_measurements_match_report_contract(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            report = load_report(_report_path_for_test(tmpdir))
